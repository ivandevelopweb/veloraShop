...
Only `success` (and LiqPay's documented `sandbox` status when Sandbox mode is explicitly enabled) transitions an order to `paid`. `error` and `failure` become `failed`; `reversed` becomes `cancelled`; non-final states remain `pending`. Repeated callbacks lock the same order and perform no second stock decrement, cart clear or state event.
